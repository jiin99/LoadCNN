...
def main():
    ...
    directory = ['7days_1/ckpt_last.pt','7days_2/ckpt_last.pt','7days_3/ckpt_last.pt','7days_4/ckpt_last.pt','7days_5/ckpt_last.pt']
    test_prec = validate(test_loader,criterion,directory,'test')
    print(test_prec)
    print(' ensemble rmse test prec : ',test_prec)
    model = cnn_simple.CNN_new(7)
    for i in directory:
        pre_trained_net = '/daintlab/home/jiin9/LoadCNN/ensemble_new_base_mae/' + i
    ...
    with torch.no_grad():
        ...
        for i, (val_x,val_y) in tqdm(enumerate(val_loader)):
            ...
            for p,k in enumerate(directory) :
                model = cnn_simple.CNN_new(7)
                pre_trained_net = '/daintlab/home/jiin9/LoadCNN/ensemble_new_base_mae/' + k
                model.load_state_dict(torch.load(pre_trained_net))
                model.eval()
